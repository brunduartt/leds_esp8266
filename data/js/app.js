...
function addButtonField(field) {
  ...
  var btngroup = template.find(".btn-group");
  btngroup.attr("id", "btn-group-" + field.name);
  var btn = template.find("#btn").clone();
  btngroup.html("");
  for (var i = 0; i < field.options.length; i++) {
    var buttonText = field.options[i];
    var button = btn.clone();
    button.text(buttonText);
    button.attr("value", i);
    button.attr("id", "btn" + field.name);
    button.click(function() {
      var value = $(this).attr("value")
      setButtonFieldValue(field, value);
    });
...
function addSelectField(field) {
  ...
  select.change(function() {
    ...
    if(field.reload) {
      postAndReload(field.name, value);
    } else {
...
function setBooleanFieldValue(field, btnOn, btnOff, value) {
  ...
  btnOff.attr("class", !field.value ? "btn btn-primary" : "btn btn-default");
  if(field.reload) {
    postAndReload(field.name, field.value);
  } else {
    postValue(field.name, field.value);
  }
}

function setButtonFieldValue(field, value) {
  field.value = value;
  if(field.reload) {
    postAndReload(field.name, field.value);
  } else {
...
function postAndReload(name, value) {
  $("#status").html("Setting with reload" + name + ": " + value + ", please wait...");
  ...
  $.post(urlBase + name + "?value=" + value, body, function(data) {
    ...
    if (data.name != null) {
      $("#status").html("Set with reload" + name + ": " + data.name);
    } else {
      $("#status").html("Set with reload" + name + ": " + data);
    }
  });
}
